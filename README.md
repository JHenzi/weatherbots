...
### Configure secrets
- Copy `.env.example` → `.env` and fill in the values you need (see **Environment variables** below).
...
| **Postgres** | | |
| `PGDATABASE_URL` or `DATABASE_URL` | No | Postgres connection URL (e.g. `postgresql://user:pass@host:5432/dbname`). Used when Postgres is enabled. |
...
### 1) Prepare `.env`
Copy `.env.example` to `.env` and set at least: `KALSHI_API_KEY_ID`, `KALSHI_PRIVATE_KEY_PATH`, `KALSHI_ENV` (e.g. `prod` or `demo`), and `NWS_USER_AGENT`. Add forecast provider keys as needed. Full list: see **Environment variables** in the Quick start section above.
